...
if [ $? = 0 ] ; then
	...
	cp -a ${PROFILE_MNT}/${PROFILENAME}/data_files ${TOOLKIT}
	...
	if [ -f ${PROFILE_MNT}/${PROFILENAME}/bootdrive.img ] ; then
		partimage restore ${BOOTDEVNODE} ${PROFILE_MNT}/${PROFILENAME}/bootdrive.img
		if [ $? = 0 ] ; then
			echo ""
